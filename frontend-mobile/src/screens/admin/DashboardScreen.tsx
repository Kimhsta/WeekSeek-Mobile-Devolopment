...
export default function DashboardScreen() {
  ...
  const popularFilms = [...films].sort((a, b) => b.views - a.views).slice(0, 3);
    ...
    <ScrollView className="flex-1 bg-gray-100 p-4">
      ...
      <View className="mb-6">
        ...
        <Text className="text-gray-600 mt-2">
          Selamat datang kembali, <Text className="font-semibold text-slate-800">{username}</Text>!
        </Text>
      </View>
      ...
      <View className="flex-row justify-between mb-6 space-x-4">
        <View className="flex-1 bg-white rounded-2xl p-4 shadow">
          <View className="flex-row items-center">
            ...
            <View className="ml-3">
              <Text className="text-lg font-semibold text-slate-800">{totalFilms}</Text>
        ...
        <View className="flex-1 bg-white rounded-2xl p-4 shadow">
          <View className="flex-row items-center">
            ...
            <View className="ml-3">
              <Text className="text-lg font-semibold text-slate-800">{totalViews}</Text>
      ...
      {latestFilm && (
        <View className="bg-white rounded-2xl p-4 shadow mb-6">
          <Text className="text-lg font-semibold text-slate-800 mb-2">Film Terbaru</Text>
          <View className="flex-row items-center">
            <Image source={{ uri: latestFilm.posterUrl }} className="w-20 h-28 rounded-md bg-gray-200" resizeMode="cover" />
            <View className="ml-4 flex-1">
              <Text className="text-base font-bold text-slate-800" numberOfLines={1}>{latestFilm.title}</Text>
              <Text className="text-sm text-gray-600 mt-1">Genre: {latestFilm.genre || '-'}</Text>
              <Text className="text-sm text-gray-600">Rilis: {latestFilm.releaseYear}</Text>
            </View>
          </View>
        </View>
      )}
      ...
      {popularFilms.length > 0 && (
        <View className="bg-white rounded-2xl p-4 shadow mb-6">
          <Text className="text-lg font-semibold text-slate-800 mb-4">Film Populer</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} className="space-x-4">
            {popularFilms.map((film) => (
              <View key={film.id} className="w-40 bg-gray-50 rounded-lg p-2 shadow">
                <Image source={{ uri: film.posterUrl }} className="w-full h-24 rounded-md mb-2 bg-gray-200" resizeMode="cover" />
                <Text className="text-sm font-semibold text-slate-800" numberOfLines={1}>{film.title}</Text>
                <Text className="text-xs text-gray-500">Views: {film.views}</Text>
              </View>
            ))}
          </ScrollView>
        </View>
      ...
      <View className="bg-white rounded-2xl p-4 shadow">
        <Text className="text-lg font-semibold text-slate-800 mb-4">Aksi Cepat</Text>
        <View className="flex-row justify-around">
          <TouchableOpacity onPress={handleAddFilm} className="items-center">
            <View className="p-4 bg-blue-100 rounded-full mb-2">
              <FontAwesome5 name="plus" size={20} color="#3B82F6" />
            </View>
            <Text className="text-sm text-slate-800">Tambah Film</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={handleDeleteFilm} className="items-center">
            <View className="p-4 bg-red-100 rounded-full mb-2">
              <MaterialIcons name="delete" size={20} color="#EF4444" />
            </View>
            <Text className="text-sm text-slate-800">Hapus Film</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={handleReport} className="items-center">
            <View className="p-4 bg-green-100 rounded-full mb-2">
              <Ionicons name="stats-chart" size={20} color="#10B981" />
            </View>
            <Text className="text-sm text-slate-800">Cetak PDF</Text>
          </TouchableOpacity>
        </View>
      </View>
    </ScrollView>
